  ...
  for (int i = 0; i < mD.headers.size(); i++) {
    ...
    switch (mD.headers[i].type) {
      case 0:
        ...
        break;
      case 4:
        cout << "commentBlockSize: " << mD.vorbisComment.commentBlockSize
             << endl;
        cout << "vendorLength: " << mD.vorbisComment.vendorLength << endl;
        cout << "vendorString: " << mD.vorbisComment.vendorString << endl;
        cout << "commentListLength: " << mD.vorbisComment.commentListLength
             << endl;
...
HeadData::metaDataBlockData HeadData::readMetaDataBlock(fileReader &input) {
  ...
  do {
    ...
    if (header.type == 0) {
      //         unsigned short minBlockSize;      // 16bit 最小块大小,
      //         单位是sample
      // unsigned short maxBlockSize;      // 16bit 最大块大小, 单位是sample
      // unsigned int minFrameSize;        // 24bit 最小帧大小, 单位是byte,
      // 0表示未知 unsigned int maxFrameSize;        // 24bit 最大帧大小，
      // 单位是byte, 0表示未知 unsigned int sampleRate;          // 20 bit
      // 采样率hz unsigned short numChannels;       // 3 bit
      // 通道数减一(通道数=numChannels+1) unsigned short sampleDepth;       // 5
      // bit 采样深度减一(采样深度=sampleDepth+1) unsigned long long
      // totalSamples;  // 36bit 总样本数 unsigned char md5[16];            //
      // 128bit MD5校验码
      ...
      metaData.streamInfo.totalSamples =
          (input.readBigUInt(32) << 4) | input.readBigUInt(4);
      for (int i = 0; i < 4; i++) {
        metaData.streamInfo.md5[i] = intToHex(input.readBigUInt(32));
      }
    } else if (header.type == 4) {
      // 实际编码时，因为向量之间没有分隔符，必须编码每个向量的长度。每个向量编码时，先将其长度作为一个有符号int整数写入，再写入向量字符串。有符号int叫做向量长度，按照Little
      // Endian顺序。因为供应商字符串必须存在，单独解码；向量可以放在循环里解码，所以在供应商字符串后有一个有符号int向量总数。向量总数不包括供应商字符串。
      ...
      metaData.vorbisComment.vendorStringOriginal.reserve(
          metaData.vorbisComment.vendorLength);
      ...
      string vendorString(metaData.vorbisComment.vendorStringOriginal.begin(),
                          metaData.vorbisComment.vendorStringOriginal.end());
      ...
      cout << "commentListLength: " << metaData.vorbisComment.commentListLength
           << endl;
...
void HeadData::editMetaDataBlock(fileReader &infile, fileWriter &outfile,
                                 metaDataEdit edit) {
  metaDataBlockData metaData;
  ...
  if (edit.isEditVendorString) {
    ...
    cout << "befor edit vendor string: " << metaData.vorbisComment.vendorString
         << endl;
    vector<unsigned int> vendorStringOriginal(
        metaData.vorbisComment.vendorString.begin(),
        metaData.vorbisComment.vendorString.end());
    metaData.vorbisComment.vendorString = edit.newVendorString;
    vector<unsigned int> myVector(metaData.vorbisComment.vendorString.begin(),
                                  metaData.vorbisComment.vendorString.end());
    metaData.vorbisComment.vendorStringOriginal = myVector;
    edit.alterSize = metaData.vorbisComment.vendorStringOriginal.size() -
                     vendorStringOriginal.size();
    cout << "after edit vendor string: " << metaData.vorbisComment.vendorString
         << endl;
  }
  if (edit.modifyComment) {
    if (edit.modifyCommentIndex == -1 ||
        edit.modifyCommentIndex >= metaData.vorbisComment.commentListLength) {
      throw runtime_error("modify comment index error");
    }
    cout << "modify comment" << endl;
    cout << "befor modify comment " << edit.modifyCommentIndex + 1 << ": "
         << metaData.vorbisComment.commentList[edit.modifyCommentIndex] << endl;

    metaData.vorbisComment.commentList[edit.modifyCommentIndex] =
        edit.modifiedComment;
    vector<unsigned int> myVector(edit.modifiedComment.begin(),
                                  edit.modifiedComment.end());
    edit.alterSize +=
        myVector.size() -
        metaData.vorbisComment.commentsOriginal[edit.modifyCommentIndex].size();
    metaData.vorbisComment.commentsOriginal[edit.modifyCommentIndex] = myVector;

    cout << "after modify comment " << edit.modifyCommentIndex + 1 << ": "
         << metaData.vorbisComment.commentList[edit.modifyCommentIndex] << endl;
  }
  if (edit.appendComment) {
    ...
    for (int i = 0; i < edit.newComments.size(); i++) {
      cout << "append comment: " << i << " line: " << edit.newComments[i]
           << endl;
      metaData.vorbisComment.commentListLength++;
      metaData.vorbisComment.commentList.push_back(edit.newComments[i]);
      vector<unsigned int> myVector(edit.newComments[i].begin(),
                                    edit.newComments[i].end());
      metaData.vorbisComment.commentsOriginal.push_back(myVector);
      edit.alterSize += myVector.size();
    }
  }
  if (edit.removeComment) {
    if (edit.removeCommentIndex == -1 ||
        edit.removeCommentIndex >= metaData.vorbisComment.commentListLength) {
      throw runtime_error("remove comment index error");
    }
    cout << "remove comment" << endl;
    cout << "remove comment " << edit.removeCommentIndex << " : "
         << metaData.vorbisComment.commentList[edit.removeCommentIndex] << endl;
    edit.alterSize -=
        metaData.vorbisComment.commentsOriginal[edit.removeCommentIndex].size();
    metaData.vorbisComment.commentList.erase(
        metaData.vorbisComment.commentList.begin() + edit.removeCommentIndex);
    metaData.vorbisComment.commentsOriginal.erase(
        metaData.vorbisComment.commentsOriginal.begin() +
        edit.removeCommentIndex);
    metaData.vorbisComment.commentListLength--;
  }
  ...
  for (int i = 0; i < metaData.headers.size(); i++) {
    ...
    if (metaData.headers[i].type == 0) {
      ...
      outfile.writeLittleInt(
          (int)(metaData.vorbisComment.vendorLength + edit.alterSize), 32);
      ...
      for (int j = 0; j < metaData.vorbisComment.commentListLength; j++) {
        outfile.writeLittleInt(metaData.vorbisComment.commentList[j].length(),
                               32);
...
HeadData::metaDataEdit getEditData(const string &modifyVendor,
                                   const string &modifyComment,
                                   const string &modifyCommentIndex,
                                   const string &appendComment,
                                   const string &removeCommentIndex) {
  ...
  if (!modifyComment.empty()) {
    ...
    if (!modifyCommentIndex.empty()) {
      ...
      throw runtime_error(
          "Error acquiring comment index for modifying (main::main)");
    }
  }
  ...
  if (!removeCommentIndex.empty()) {
    ...
    if (!removeCommentIndex.empty()) {
      ...
      throw runtime_error(
          "Error acquiring comment index for removing (main::main)");
    }
  }
  return metaEditInfo;
}
