...
struct WAVHeader read_header() {
  ...
  //      printf("Input the sub format (note: Input 16 bytes in decimal using
  //      little-endian byte order, separated by spaces): "); for (int i = 0; i
  //      < 16; i++) {
...
void raw_to_wav(struct WAVHeader header) {
  ...
  if (header.has_fact_chunk) {
    ...
    write_bytes_little(header.fo, &header.fact_chunk_data,
                       header.fact_chunk_size);
  }
  // Write data chunk
...
struct WAVHeader read_wav(FILE *f) {
  ...
  if (strcmp(buf, "fact") == 0) {
    ...
    read_bytes_little(header.fi, &header.fact_chunk_data,
                      header.fact_chunk_size);
  }
  // Read data chunk
...
void wav_to_flac() {
  ...
  // Pack rem, number of channels and bits per sample to gather (4 + 3 + 5 = 12
  // bits)
  tmp = (rem << 8) + ((wav_header.channels - 1) << 5) +
        (wav_header.bits_per_sample - 1);
  ...
  while (1) {
    // Write sub frame
    uint32_t num = fread(chunk_buf, 1, block_samples * wav_header.block_align,
                         wav_header.fi);
    if (num == 0) {
      break;
    }
    ...
    for (int i = 0; i < samples; i++) {
      for (int j = 0; j < 2; j++) {
        ...
        sample_buf[j][i] =
            (sample_buf[j][i] << (32 - wav_header.bits_per_sample)) >>
            (32 - wav_header.bits_per_sample);
      }
    }
    // Write channels
