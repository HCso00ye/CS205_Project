...
void decodeFile(fileReader &in, fileWriter &out, pcmConfig &config)
{
    ...
    {
        throw runtime_error(
            "Unrecognized WAV file chunk 'fmt ' (Wav2flac::encodeFile)");
    }
    ...
    {
        throw runtime_error(
            "Invalid WAV file channel count (Wav2flac::encodeFile)");
    }
    ...
    {
        throw runtime_error(
            "Invalid WAV file data chunk header 'data' (Wav2flac::encodeFile)");
    }

    // check data chunk size
    int sampleDataLen = in.readLittleUInt(32);
    if (sampleDataLen <= 0 ||
        sampleDataLen % (numChannels * (sampleDepth / 8)) != 0)
    {
